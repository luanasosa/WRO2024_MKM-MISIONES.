...
void estrategiaIzquierda() {
  if (measureDistance() < 65) { // Si hay un obstáculo adelante
    Serial.println("Estrategia izquierda");
    analogWrite(motorPWMPin, 0);  // STOP
    delay(100);
    digitalWrite(motorDirPin, LOW);  // Retroceder
    analogWrite(motorPWMPin, 60);  // Retroceder a la velocidad base
    delay(2000);
    digitalWrite(motorDirPin, HIGH);  // Restablecer la dirección del motor para avanzar
    smoothServoMove(180); // Girar a la izquierda
    delay(1000); // Esperar para girar
    adjustMotorSpeed(40); // Avanzar a velocidad base
    delay(1000);
  }
}
...
void estrategiaDerecha() {
  if (measureDistance() < 60) { // Si hay un obstáculo adelante
    Serial.println("Estrategia derecha");
    analogWrite(motorPWMPin, 0);  // STOP
    delay(100);
    digitalWrite(motorDirPin, LOW);  // Retroceder
    analogWrite(motorPWMPin, 60);  // Retroceder a la velocidad base
    delay(2000);
    digitalWrite(motorDirPin, HIGH);  // Restablecer la dirección del motor para avanzar
    smoothServoMove(70); // Girar a la derecha
    delay(1000); // Esperar para girar
    adjustMotorSpeed(40); // Avanzar a velocidad base
  }
}
